...
writeLines(c(sprintf("Procedure types among the %d interventions: %s.", iv$n_interv, paste(sprintf("%s %d", names(iv$modality), as.integer(iv$modality)), collapse="; ")),
             ...
             sprintf("Cumulative mortality after the echocardiogram showing dysfunction: %s at 1 year, %s at 3 years, %s at 5 years (Kaplan-Meier; median follow-up %.1f years by reverse Kaplan-Meier; survivors censored one year after the last hospital discharge, the period for which MIMIC-IV records deaths, or at the last echocardiogram if later).", paste(sprintf("%.1f%% (%.1f–%.1f)", 100*mort$cum_mortality, 100*mort$lo, 100*mort$hi)[1]), paste(sprintf("%.1f%% (%.1f–%.1f)", 100*mort$cum_mortality, 100*mort$lo, 100*mort$hi)[2]), paste(sprintf("%.1f%% (%.1f–%.1f)", 100*mort$cum_mortality, 100*mort$lo, 100*mort$hi)[3]), r2$followup_after_dysfunction_median_years),
             sprintf("Reproduced endpoint, counted from the reproducing echocardiogram: %d patients (%d moderate or severe at index plus reproduced events); %d with dysfunction at or after it; %d without a prior procedure; %d interventions; %d died.", ic["n_modplus"], ic["n_index"], ic["n_pheno"], ic["n_analysed"], ic["n_interv"], ic["died"])),
...
wcsv(r1$confirmed_timing, "tableS11_confirmed_timing_full.csv"); wcsv(r1$dx_timing, "tableS11_diagnosis_timing_full.csv"); wcsv(r1$confirmed_firstonly, "tableS11_confirmed_first_report_only_full.csv")
...
cx <- r1$main[r1$main$domain=="D3_cox_companion",]; m3 <- r1$main[r1$main$domain=="D3",]; m5 <- r1$main[r1$main$domain=="D5",]; ct <- r1$confirmed_timing; dxp <- r1$dx_timing; cfo <- r1$confirmed_firstonly
...
S11 <- do.call(rbind, lapply(c("onset","progression"), function(st) do.call(rbind, lapply(c("first","confirmed"), function(de) do.call(rbind, lapply(c(ord[1:15], "dm","htn","cad"), function(t) {
  ...
  data.frame(cohort=st, definition=ifelse(de=="first","first-observed","confirmed"), characteristic=unname(lab[t]),
             ...
             confirmed_first_report_only=if (de=="confirmed" && !cod) g2(cfo,st,"confirmed, first report only",t) else "",
...
writeLines(c(sprintf("Main model and companions fitted in the same patients: onset %s (first-observed), %s (confirmed); progression %s (first-observed), %s (confirmed).", cnt(m3,"onset","first"), cnt(m3,"onset","confirmed"), cnt(m3,"progression","first"), cnt(m3,"progression","confirmed")),
             sprintf("Reproduced events dated at the reproducing echocardiogram: onset %s; progression %s. Reproduced endpoint restricted to the first moderate or severe report (follow-up ends at the first report): onset %s; progression %s.", cnt(ct,"onset","confirmed, dated at the confirming echocardiogram"), cnt(ct,"progression","confirmed, dated at the confirming echocardiogram"), cnt(cfo,"onset","confirmed, first report only"), cnt(cfo,"progression","confirmed, first report only")),
